...
function PlanToday() { 
  ...
  const difference=(x,y)=>{
    ...
    const milli= diffMinutes*60 *1000;
    return milli;
  ...
  return (
    < >
    <div className="bg-gradient-to-b from-[#510A32] to-[#801336] p-4 text-[#2D142C]">
       <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-[#C72C41] min-h-[80vh] md:w-[70%]">
        <h1 className='font-bold text-center text-3xl'>Set Targets for TODAY</h1>
         <div className="addTodo my-5 flex flex-col gap-4">
          ...
          <input  onChange={handleChangeFrom} value={todoFrom} type="time" className='w-[15%] rounded-full px-5 py-1 bg-gradient-to-r from-[#FF847C] to-[#FECEA8]' />
          <input  onChange={handleChangeTo} value={todoTo} type="time" className='w-[15%] rounded-full px-5 py-1 bg-gradient-to-r from-[#FF847C] to-[#FECEA8]' />
         ...
         <h2 className='text-2xl font-bold'>Your Targets for today are - </h2>
         <div className="todos">
          ...
          return (showFinished || !item.isCompleted) && <div key={item.id} className={"todo flex my-3 justify-between"}>
            ...
            <div className={item.isCompleted ? "line-through" : ""}>
                  <span className="flex justify-between w-full gap-5">
                    ...
                    <span><TIMER duration={difference(item.todoFrom,item.todoTo)}/></span>
